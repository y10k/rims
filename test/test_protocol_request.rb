...
body[]
    ...
    def test_scan_line(data)
      expected_atom_list, line = data
      assert_equal(expected_atom_list, @reader.scan_line(line))
      assert_equal('', @output.string)
    end

    def test_scan_line_string_literal
      line = 'A003 APPEND saved-messages (\Seen) ' + "{#{LITERAL_1.bytesize}}"
      @input.string = LITERAL_1 + "\r\n"

      assert_equal([ 'A003', 'APPEND', 'saved-messages', '('.intern, '\Seen', ')'.intern, LITERAL_1 ], @reader.scan_line(line))
